...
public final class LinkCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender s, Command c,
        ...
        switch (a[0].toLowerCase(Locale.ROOT)) {
            ...
            case "pets" -> {
                // Aceita um jogador como alvo para poder ser usado do console:
                // quem esta ao lado do bicho costuma ser a conta Bedrock, que
                // normalmente nao e op e nao consegue rodar isto sozinha.
                Player p = targetOf(s, a, 1);
                if (p == null) return true;
                int r = a.length >= 3 ? parseInt(a[2], 10) : (a.length >= 2 && !(s instanceof Player) ? 10 : (a.length >= 2 ? parseInt(a[1], 10) : 10));
            ...
            case "claimpets" -> {
                Player p = targetOf(s, a, 1);
                if (p == null) return true;
                if (groups.of(p.getUniqueId()) == null) { err(s, p.getName() + " is not in a linked group."); return true; }
                int r = a.length >= 3 ? parseInt(a[2], 10) : (a.length >= 2 && !(s instanceof Player) ? 10 : (a.length >= 2 ? parseInt(a[1], 10) : 10));
            ...
            case "retargetpets" -> {
                ...
                if (g == null) { err(s, p.getName() + " is not in a linked group."); return true; }
                engine.retargetPets(g, p);
                ok(s, "pets around the world reassigned to " + p.getName() + ".");
            }
            case "reload" -> {
                plugin.reloadAll();
    ...
    /**
     * Resolve em quem o comando age.
     *
     * No console e obrigatorio nomear o jogador; em jogo, o proprio executor e
     * o alvo, e nomear outro continua valendo. Isso e o que permite a um admin
     * agir pela conta Bedrock, que costuma nao ter permissao para rodar nada.
     */
    private Player targetOf(CommandSender s, String[] a, int idx) {
        if (a.length > idx) {
            Player byName = Bukkit.getPlayerExact(a[idx]);
            if (byName != null) return byName;
            if (!(s instanceof Player)) {
                err(s, "'" + a[idx] + "' is not online.");
                return null;
            }
        }
        if (s instanceof Player self) return self;
        err(s, "from console, name the player: /" + "crosslink " + a[0] + " <player> [radius]");
        return null;
    }
    ...
    private void usage(CommandSender s, String label) {
        ...
        info(s, "/" + label + " pets [player] [radius]      - who owns the animals around them");
        info(s, "/" + label + " claimpets [player] [radius]  - adopt them into the group");
        info(s, "/" + label + " retargetpets [player]        - reassign this group's pets world-wide");
    ...
    public List<String> onTabComplete(CommandSender s, Command c,
        ...
        if (a.length == 1) {
            return filter(List.of("create", "add", "remove", "sync", "primary", "backups",
                    "restore", "resetprompt", "pets", "claimpets", "retargetpets", "list", "delete", "reload"), a[0]);
        }
        if (a.length == 2) {
            if (a[0].equalsIgnoreCase("sync") || a[0].equalsIgnoreCase("restore")
                    || a[0].equalsIgnoreCase("backups") || a[0].equalsIgnoreCase("resetprompt")
                    || a[0].equalsIgnoreCase("pets") || a[0].equalsIgnoreCase("claimpets")
                    || a[0].equalsIgnoreCase("retargetpets")) {
